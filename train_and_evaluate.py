...
# ensure local directory is on path for imports
...
# Attack names mapping - kept for evaluation mapping
ATTACK_NAMES = {
...
class IntegratedTrainer:
    ...
    def train(self):
        ...
        # Because model has two outputs (embedded_image, output_watermark),
        # map losses and metrics explicitly by output name
        self.model.compile(
            ...
            metrics={
                'embedded_image': ['mse', 'mae'],
                'output_watermark': ['accuracy']
            }
        )
        ...
        # Print a concise training summary (last metrics)
        print("\n" + "=" * 40)
        print("TRAINING SUMMARY (last epoch metrics)")
        print("=" * 40)
        hist = history.history
        # Show keys and last values
        ...
        model_path = os.path.join(self.output_dir, 'final_model_weights.keras')
        # use save_weights to keep file small
    ...
    def evaluate(self):
        ...
        # Find and save only the best result
        best_result = max(results, key=lambda r: (r['psnr'] + r['ssim'] * 100 - r['ber']))
        print(f"\n✓ Best result: {best_result['image_name']} with {best_result['attack_name']}")
        print(f"  PSNR: {best_result['psnr']:.2f} dB | SSIM: {best_result['ssim']:.4f} | BER: {best_result['ber']:.2f}%")
        self._save_visualization(best_result)
    ...
    def _save_visualization(self, result):
        ...
        # Original Watermark
        watermark_img = result['watermark'].reshape(int(np.sqrt(result['watermark'].size)),
                                                    int(np.sqrt(result['watermark'].size)))
        ...
        # Extracted Watermark
        extracted_img = result['extracted_watermark'].reshape(int(np.sqrt(result['extracted_watermark'].size)),
                                                              int(np.sqrt(result['extracted_watermark'].size)))
...
{result['quality']}
    ...
    def _generate_summary(self, results):
        ...
        for attack_id in range(len(ATTACK_NAMES)):
            ...
            if attack_results:
                attack_stats[ATTACK_NAMES[attack_id]] = {
                    'avg_psnr': float(np.mean([r['psnr'] for r in attack_results])),
                    'avg_ssim': float(np.mean([r['ssim'] for r in attack_results])),
                    'avg_ber': float(np.mean([r['ber'] for r in attack_results])),
                    'count': len(attack_results)
                }

        # Create summary visualization (safe: use matplotlib default colors)
        ...
        # PSNR
        axes[0].bar(range(len(attack_names)), psnr_values)
        ...
        axes[0].axhline(y=35, linestyle='--', label='Good (35 dB)')
        axes[0].axhline(y=30, linestyle='--', label='Fair (30 dB)')
        axes[0].legend()
        axes[0].grid(axis='y', alpha=0.3)

        # SSIM
        axes[1].bar(range(len(attack_names)), ssim_values)
        ...
        axes[1].axhline(y=0.94, linestyle='--', label='Good (0.94)')
        axes[1].legend()
        axes[1].grid(axis='y', alpha=0.3)

        # BER
        axes[2].bar(range(len(attack_names)), ber_values)
        ...
        axes[2].axhline(y=5, linestyle='--', label='Good (5%)')
        axes[2].legend()
        ...
        overall_ber = np.mean([r['ber'] for r in results])

        print(f"\nTotal Tests: {len(results)}")
        print(f"Number of Attacks Tested: {len(attack_stats)}")
        print(f"\nOVERALL METRICS:")
        print(f"  Average PSNR: {overall_psnr:.2f} dB")
        print(f"  Average SSIM: {overall_ssim:.4f}")
        print(f"  Average BER:  {overall_ber:.2f}%")
